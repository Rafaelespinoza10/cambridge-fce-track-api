...
// Same per-Lambda singleton pattern as every other AI-backed composition in
// this codebase — reused across warm invocations of THIS function only.
...
interface MistakeRemixGenerationServices {
  generateMistakeRemix: GenerateMistakeRemixExerciseService;
}

async function buildMistakeRemixGenerationServices(): Promise<MistakeRemixGenerationServices> {
  ...
  return {
    generateMistakeRemix: new GenerateMistakeRemixExerciseService(dataSource, {
      llm,
...
export { buildMistakeRemixGenerationServices };
export type { MistakeRemixGenerationServices };
